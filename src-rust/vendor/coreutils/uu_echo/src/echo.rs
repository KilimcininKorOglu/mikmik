...
use std::io::Write;
// MikMik patch: the redirectable stand-in for `uucore::streams::StdoutLock`.
use uucore::streams::StdoutLock;
...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
    ...
    let (args, options): (Box<dyn Iterator<Item = OsString>>, Options) = if is_posixly_correct {
        ...
        if first_arg == "--help" && args.peek().is_none() {
            ...
            writeln!(uucore::streams::stdout(), "echo {}", crate_version!())?;
            return Ok(());
        }
    ...
    execute(&mut uucore::streams::stdout().lock(), args, options)?;
