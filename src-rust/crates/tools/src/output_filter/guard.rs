//! Never-worse output guard: the filter never emits more tokens than the raw command.
//!
//! Ported from RTK (Rust Token Killer). A filter that reformats output can, in
//! pathological cases, make it larger; this guard reverts to the raw output when
//! that happens, so filtering can only ever help.
